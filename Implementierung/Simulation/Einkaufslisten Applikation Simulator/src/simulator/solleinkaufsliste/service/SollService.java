...
public class SollService {
	...
	// kumuliere Verbrauchsablauf in Abh�ngigkeit zum einkaufsZeitpunkt
	...
	// finde den gr��ten Zahleintrag einer Liste (Verbrauchsablauf), der <= einer
	// angegebenen Zahl (Einkaufzeitpunkt).
	...
	// kleinstes Element was gr��er oder gleich einen Wert ist
	...
	// finde den index eines Werts (Einkaufzeitpunkt) aus einer Liste
	// (Einkaufverbrauch)
	...
	// erstelle eine Persona mit verbrauchsdaten (Einkaufsablauf und
	// Verbrauchsablauf sind kummuliert)
	/****************
	 * WENN ALLEN PERSONAS IMMER DIE GLEICHEN PRODUKTE ZUGEWIESEN SIND BRAUCHT MAN
	 * KEINE IF BEDINGUNG
	 *****************/
	public Persona erstellePersonaMitVerbrauchsdaten(String personaBezeichnung, int einkaufRangeMin,
			int einkaufRangeMax, int anzahlDatensaetze) {
		...
		persona.setEinkaufsAblauf(this.kumuliereEinkaufsablauf(
				this.erstelleVerbrauchsOderEinkaufsablauf(einkaufRangeMin, einkaufRangeMax, anzahlDatensaetze)));
		...
		List<Integer> butterNormalVerbrauchsverlauf = this.erstelleVerbrauchsOderEinkaufsablauf(butter.getRangeMin(),
				butter.getRangeMax(), anzahlDatensaetze);
		List<Integer> milchNormalVerbrauchsverlauf = this.erstelleVerbrauchsOderEinkaufsablauf(milch.getRangeMin(),
				milch.getRangeMax(), anzahlDatensaetze);
		List<Integer> kaeseNormalVerbrauchsverlauf = this.erstelleVerbrauchsOderEinkaufsablauf(kaese.getRangeMin(),
				kaese.getRangeMax(), anzahlDatensaetze);
		List<Integer> salamiNormalVerbrauchsverlauf = this.erstelleVerbrauchsOderEinkaufsablauf(salami.getRangeMin(),
				salami.getRangeMax(), anzahlDatensaetze);
		List<Integer> JoghurtNormalVerbrauchsverlauf = this.erstelleVerbrauchsOderEinkaufsablauf(Joghurt.getRangeMin(),
				Joghurt.getRangeMax(), anzahlDatensaetze);
		...
		butter.setVerbrauchsablauf(
				this.kumuliereVerbrauchsablauf(butterNormalVerbrauchsverlauf, persona.getEinkaufsAblauf()));
		milch.setVerbrauchsablauf(
				this.kumuliereVerbrauchsablauf(milchNormalVerbrauchsverlauf, persona.getEinkaufsAblauf()));
		kaese.setVerbrauchsablauf(
				this.kumuliereVerbrauchsablauf(kaeseNormalVerbrauchsverlauf, persona.getEinkaufsAblauf()));
		salami.setVerbrauchsablauf(
				this.kumuliereVerbrauchsablauf(salamiNormalVerbrauchsverlauf, persona.getEinkaufsAblauf()));
		Joghurt.setVerbrauchsablauf(
				this.kumuliereVerbrauchsablauf(JoghurtNormalVerbrauchsverlauf, persona.getEinkaufsAblauf()));
	...
	public List<Produkt> erstelleSollEinkaufsListe(Persona persona, int einkaufZeitpunkt) {
		...
		int aktuellerButterVerbrauchsablauf = this.findeGroesteZahlKleinerGleichKaufzeitpunkt(butterVerbrauchsablauf,
				einkaufZeitpunkt);
		int aktuellerMilchVerbrauchsablauf = this.findeGroesteZahlKleinerGleichKaufzeitpunkt(milchVerbrauchsablauf,
				einkaufZeitpunkt);
		int aktuellerKaeseVerbrauchsablauf = this.findeGroesteZahlKleinerGleichKaufzeitpunkt(kaeseVerbrauchsablauf,
				einkaufZeitpunkt);
		int aktuellerSalamiVerbrauchsablauf = this.findeGroesteZahlKleinerGleichKaufzeitpunkt(salamiVerbrauchsablauf,
				einkaufZeitpunkt);
		int aktuellerJoghurtVerbrauchsablauf = this.findeGroesteZahlKleinerGleichKaufzeitpunkt(JoghurtVerbrauchsablauf,
				einkaufZeitpunkt);

		// Logging von
//		System.out.println("Einkaufszeitpunkt: " + einkaufZeitpunkt);
...
//		System.out.println("aktuellerJoghurtVerbrauchsablauf: " + aktuellerJoghurtVerbrauchsablauf);
		...
		if (aktuellerButterVerbrauchsablauf < einkaufZeitpunkt) {
			...
			if (aktuellerEinkaufsablaufIndex <= einkaufZeitpunkt && aktuellerEinkaufsablaufIndex != 0) {
				einkaufZeitpunktDavor = einkaufsAblauf.get(aktuellerEinkaufsablaufIndex - 1);
			}
			//System.out.println("einkaufZeitpunktDavor Butter: " + einkaufZeitpunktDavor);
		...
		if (aktuellerMilchVerbrauchsablauf < einkaufZeitpunkt) {
			...
			//System.out.println("einkaufZeitpunktDavor Milch: " + einkaufZeitpunktDavor);
		...
		if (aktuellerKaeseVerbrauchsablauf < einkaufZeitpunkt) {
			...
			//System.out.println("einkaufZeitpunktDavor Kaese: " + einkaufZeitpunktDavor);
		...
		if (aktuellerSalamiVerbrauchsablauf < einkaufZeitpunkt) {
			...
			//System.out.println("einkaufZeitpunktDavor Salami: " + einkaufZeitpunktDavor);
		...
		if (aktuellerJoghurtVerbrauchsablauf < einkaufZeitpunkt) {
			...
			//System.out.println("einkaufZeitpunktDavor Joghurt: " + einkaufZeitpunktDavor);
	...
	/**
	 * erstelle SollEinkaufslisten vor jedem Einkauf �ber ein Zeitraum in Tagen. Da
	 * es verkommen kann, das zum Zeitpunkt eines Einkaufs die Solleinkaufsliste
	 * leer ist, gibt diese Methode nur ausgef�llte sollEinkaufslisten
	 * 
	 * @param persona
	...
	/**
	 * Speichert ein initialisiertes Persona Object in der Datei persona.ser
	 * 
	 * @param personaBezeichnung
	 * @param einkaufRangeMin
	 * @param einkaufRangeMax
	 * @param anzahlDatensaetze
	 */
	public void serializierePersona(String personaBezeichnung, int einkaufRangeMin, int einkaufRangeMax,
			int anzahlDatensaetze) {
		Persona persona = this.erstellePersonaMitVerbrauchsdaten(personaBezeichnung, einkaufRangeMin, einkaufRangeMax,
				anzahlDatensaetze);
		try {
			FileOutputStream fileOut = new FileOutputStream(
					"C:\\Users\\tchwangnwou\\Desktop\\HS REUTLINGEN\\Simulation_Dateien\\persona.ser");
	...
	public Persona deserializierePersona() {
		Persona persona = null;
		try {
			// Reading the object from a file
			FileInputStream fileIn= new FileInputStream("C:\\Users\\tchwangnwou\\Desktop\\HS REUTLINGEN\\Simulation_Dateien\\persona.ser");
	 ...
	 * @return gibt Kauftagen und deren entsprechende SollEinkaufsliste zur�ck, wenn
	 *         die SollEinkaufsliste nicht leer ist.
	 */
	public Map<Integer, List<Produkt>> findeNurAusgefuellteSollEinkaufslisten(
			Map<Integer, List<Produkt>> sollEinkaufsListenMap) {
	...
	/**
	 * sortiere eine Liste, die keyListe von der solleinkaufslisteMap, so dass die
	 * Map-Werte �ber den Key sortiert abgerufen werden k�nnen.
	 * 
	 * @param sollEinkaufsListenMap
	 * @return
	 */
	...
	public void printSollEinkaufsListen(Map<Integer, List<Produkt>> sollEinkaufsListenMap) {
		...
		System.out.println("");
		System.out.println(
				"-------------------------------Solleinkaufslisten beim Einkauf---------------------------------");
		System.out.println("");
		for (int i = 0; i < sollListeMapKey.size(); i++) {
